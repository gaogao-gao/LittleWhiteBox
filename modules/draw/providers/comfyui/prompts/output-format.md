...
images:
  - index: 1
    ...
    scene: (nsfw), (人数计数+性关系), (视角构图)
    background: (环境地点+物件), (光影+氛围)
    characters:
      - name: 角色名
        danbooru: character_name_(series) 或 name_(original) 或 ""
        type: girl|boy|woman|man|other （仅未知角色）
        appear: 发长, 发色, 瞳色, 胸部(large breasts/flat chest等) Tags （仅未知角色）
        costume: 完整服装/配饰Tags
        action: 姿态, 动作, 表情Tags
        interact: 互动Tags，纯标签（如 fellatio, holding hands），不加前缀
        uc: 排除Tags （互斥/不可见/多角色互斥）
        position: 自然语言位置词（如 in center, in left side, in right side）
```
...
scene 字段仅包含构图/相机/分级；background 字段仅包含环境/光影/氛围。详见 TAG 编写指南。
...
### 角色数量 & 关系
- 人数用 Danbooru 计数 tag: `solo`, `1girl`, `2girls`, `1boy 1girl`, `1boy 2girls`, `multiple girls`, `6+girls`
- 性关系: `hetero`, `yuri`, `yaoi`
- 相对位置: `side by side`, `above`, `below`, `surrounding`
...
### 外貌 (appear) — 仅未知角色
- 核心: 发长, 发色, 瞳色, 胸部 (large breasts / medium breasts / small breasts / flat chest)
...
### 权重调节
格式: `(tag)` / `(tag:1.2)` / `(tag:0.8)`（CLIP 标准权重语法）
...
- Known characters (已录入角色): output name + danbooru + costume + action + interact + uc + position only (禁止输出 type/appear，系统自动注入；若提供服装参考，只把你最终选定并按剧情调整后的当前服装写进 costume)
- Unknown characters: always include ALL fields: type + appear + costume + action + interact + uc + position
...
images:
  - index: 1
    ...
    scene: nsfw, solo, 1girl, third-person view, from front, low-angle shot, cowboy shot, mid shot, between legs, dutch angle, depth of field
    background: outdoors, alley, brick wall, graffiti wall, utility pole, (trash can:0.6), wet ground, late at night, pink neon light, sidelighting, dramatic shadows
    characters:
      - name: 藤原千花
...
### 示例2: POV原创角色 (1boy 1girl, 未知角色需完整字段)
```yaml
images:
  - index: 2
    anchor: 秋秋跪在地板上，抬头望着你…
    scene: nsfw, 1boy 1girl, hetero, pov, from above, high-angle shot, upper body, close-up, face focus, dynamic angle, blurry background
    background: indoors, living room, wooden floor, (window:0.8), (curtains:0.6), breeze, night, (warm lighting:0.6), sidelighting, dramatic shadows
    characters:
      - name: 秋秋
